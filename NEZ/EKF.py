...
FRAME_HEIGHT      = frame.shape[0]    # image height in pixels
SENSOR_HEIGHT_MM  = 4.712   # Pi HQ camera sensor height in mm
FOCAL_LENGTH_MM   = 16      # lens focal length in mm
DROP_DIST_M       = 2.0     # approximate distance from camera to drop in meters

# Physical parameters
G_M_S2    = 9.81           # gravity in m/s²
C_OVER_M  = 0.1            # drag coefficient over mass in 1/m
...
c_over_m_pix  = C_OVER_M  * px_per_m      # (1/m)*(px/m) → 1/s
...
class EKFTracker:
    ...
    """
    def __init__(self, dt, c_over_m_pix, g_pix, process_var, meas_var, deg_per_px):
        self.dt         = dt
...
if __name__ == '__main__':
    ...
    # mock measurement loop
    current_tilt_deg = 5.0
    measured_pixel   = 200

    # predict and update
    ekf.predict()
    ekf.update(measured_pixel, camera_angle_deg=current_tilt_deg)

    # fetch and display
    p_px, v_px_s = ekf.get_state()
    print(f"Estimated position: {p_px:.1f} px | velocity: {v_px_s:.1f} px/s "
          f"(~{v_px_s*deg_per_px:.2f} °/s)")
